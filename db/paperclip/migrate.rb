...
doc.root.elements["templates"].select{|t| t.is_a?(REXML::Element)}.each do |row|
  ...
  cells = [
    ...
    %Q("#{(row.elements["title"].text || "").gsub('"', '\"')}"),
    %Q("#{(row.elements["description"].text || "").gsub('"', '\"')}"),
    %Q('#{(row.elements["height"].text || "").gsub("'", "\'")}'),
    %Q('#{(row.elements["width"].text || "").gsub("'", "\'")}'),
    ActiveRecord::Base.connection.quote(row.elements["pdf_markup"].text || ""),
    ActiveRecord::Base.connection.quote(row.elements["form_markup"].text || ""),
    row.elements["status"].text,
